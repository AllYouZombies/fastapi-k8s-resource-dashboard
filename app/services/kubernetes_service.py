                    ...
                    for container in pod.spec.containers:
                        ...
                        if container.resources:
                            if container.resources.requests:
                                ...
                                container_info['requests']['cpu'] = self._parse_cpu(cpu_req)
                                container_info['requests']['memory'] = self._parse_memory(mem_req)

                            if container.resources.limits:
                                cpu_limit = container.resources.limits.get('cpu', '0')
                                mem_limit = container.resources.limits.get('memory', '0')
                                container_info['limits']['cpu'] = self._parse_cpu(cpu_limit)
                                container_info['limits']['memory'] = self._parse_memory(mem_limit)
