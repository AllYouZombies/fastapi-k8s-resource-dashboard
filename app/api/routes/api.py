...
async def get_chart_data(
    ...
    recent_metrics = db.query(ResourceMetric)\
        ...
        .limit(hours * 12)\
        .all()  # 12 points per hour (every 5 minutes)
    ...
    for time_key in sorted(time_groups.keys()):
        ...
        avg_cpu = sum(m.cpu_usage_cores or 0 for m in metrics) / len(metrics) if metrics else 0
        avg_memory = sum(m.memory_usage_bytes or 0 for m in metrics) / len(metrics) / (1024**2) if metrics else 0  # Convert to MB
...
async def get_summary(
    ...
    query = db.query(ResourceSummary).filter(
        ResourceSummary.timestamp == db.query(func.max(ResourceSummary.timestamp)).scalar()
    )
